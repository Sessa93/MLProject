...
\section{Problem Formulation}
  The overall problem can be divided into to main section:\newline
  The first one is the issue of features selection, what follows is a possible formal formulation of the problem:\newline
  We start from a number of features $ M = 1000 $, the objective is to find $M' \ll M$ such that 
  the selected $M'$ features gives the smallest expected generalization error, in mathematical terms:\newline\newline
  ...
  elementwise product, $V (.,.)$ is a generic loss function} \newline
  
  The problem considered is a classical of supervised learning: multi-class classification. As described in section 4.1, 
  each sample in the dataset belong to one of ten different classes 
